...
def mask_email(email):
    # Mask the email to show only the first 2 characters and domain
    if email:
...
def retrieve_messages():
    """Fetch latest messages from database on every page refresh."""
    with app.app_context():
        # Explicitly fetch required fields before expiring session
        ...
        messages_list = dbSession.query(
            ...
            Message.email,
            Message.message,
            Message.screenshot  # Ensure screenshot is preloaded
        ).order_by(Message.date_added).all()
        sanitized_messages = [
            {
                'id': m.id,
                'name': bleach.clean(m.name, strip=True),
                'email': bleach.clean(mask_email(decrypt_kenzie(aes_key_kenzie, m.email)) or '', strip=True),
                'message': bleach.clean(decrypt_kenzie(aes_key_kenzie, m.message) or '', strip=True),
                'screenshot': m.screenshot  # This should now be available
