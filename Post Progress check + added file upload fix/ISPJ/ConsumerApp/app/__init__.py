...
from Forms.appForms import LoginForm, RegistrationForm, TwoFactorForm, CreateMessageForm
...
from sqlalchemy.exc import SQLAlchemyError
from database import engine, Base, dbSession
from DBcreateTables import User, Twofa, Message, CommentFailure, File, delete_tables, create_tables
from Forms.appForms import LoginForm, RegistrationForm, TwoFactorForm, CreateMessageForm
from functools import wraps
...
# Kenzie Upload Configuration
KENZIE_UPLOAD_FOLDER = os.path.join('ISPJ', 'ConsumerApp', 'app', 'static', 'uploads')
KENZIE_ALLOWED_EXTENSIONS = {'png', 'jpg', 'gif', 'jpeg'}
app.config['KENZIE_UPLOAD_FOLDER'] = KENZIE_UPLOAD_FOLDER
app.config['KENZIE_MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16 MB

# Ensure the Kenzie upload folder exists
os.makedirs(KENZIE_UPLOAD_FOLDER, exist_ok=True)

def kenzie_allowed_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in KENZIE_ALLOWED_EXTENSIONS
...
# Debugging step: Print to check if it's loading
if not key_hex_kenzie:
    raise ValueError("AES_KEY_kenzie is not set! Check your .env file.")
aes_key_kenzie = bytes.fromhex(key_hex_kenzie)


def encrypt_kenzie(key, data):
    cipher = AES.new(key, AES.MODE_CBC)
    padded_data = pad(data.encode('utf-8'), AES.block_size)
    encrypt_data = cipher.encrypt(padded_data)
    return base64.b64encode(cipher.iv + encrypt_data)
...
def log_failure_kenzie(ip_address, reason):
    failure = dbSession.query(CommentFailure).filter_by(ip_address=ip_address, reason=reason).first()
    if failure:
        failure.failure_count += 1
        failure.last_failed_at = datetime.utcnow()
    else:
        failure = CommentFailure(ip_address=ip_address, reason=reason, failure_count=1)
        dbSession.add(failure)
    dbSession.commit()
    logger.info(f"Logged failure: IP={ip_address}, Reason={reason}")


# Updating Kenzie Upload Usage
...
def create_message():
    ...
    if form.validate_on_submit():
        ...
        if not verify_response.get("success", False) or verify_response.get("score", 0) < 0.5:
            log_failure_kenzie(request.remote_addr, "reCAPTCHA verification failed")
            flash("reCAPTCHA verification failed. Please try again.", "danger")
            return render_template('contactus.html', form=form, site_key=app.config['RECAPTCHA_PUBLIC_KEY'])

        if '@' not in form.email.data:
            log_failure_kenzie(request.remote_addr, "Invalid email address provided.")
            flash('Invalid email address. Please enter a valid email.', 'danger')
            return render_template('contactus.html', form=form, site_key=app.config['RECAPTCHA_PUBLIC_KEY'])

        encrypted_email = encrypt_kenzie(aes_key_kenzie, form.email.data)
        encrypted_message = encrypt_kenzie(aes_key_kenzie, form.message.data)
        

        screenshot = None
        
        max_size = app.config['KENZIE_MAX_CONTENT_LENGTH']
        max_size_bytes = app.config['KENZIE_MAX_CONTENT_LENGTH']
        max_size_mb = max_size_bytes / (1024 * 1024)  # Convert max allowed size to MB

        if request.content_length and request.content_length > app.config['KENZIE_MAX_CONTENT_LENGTH']:
            request_size_mb = request.content_length / (1024 * 1024)  # Convert to MB
            error_message = f"File upload exceeded limit: {request_size_mb:.2f}MB (Max: {max_size_mb:.2f}MB)"
            logger.error(error_message, extra={"ip": request.remote_addr})
            log_failure_kenzie(request.remote_addr, error_message)
            flash(f"File size exceeds the allowed limit ({max_size_mb:.2f}MB).", "danger")
            return redirect(request.url)


        if 'screenshot' in request.files:
            file = request.files['screenshot']
            if file and kenzie_allowed_file(file.filename):
                
                if file.content_length > app.config['KENZIE_MAX_CONTENT_LENGTH']:
                    error_message = f"Screenshot file exceeded limit: {file_size_mb:.2f}MB (Max: {max_size_mb:.2f}MB)"
                    logger.error(error_message, extra={"ip": request.remote_addr})
                    log_failure_kenzie(request.remote_addr, error_message)
                    flash(f"Screenshot file exceeds the allowed limit ({max_size_mb:.2f}MB).", "danger")
                    return redirect(request.url)
                try:
                    filename = generate_unique_filename(file.filename)
                    file.save(os.path.join(app.config['KENZIE_UPLOAD_FOLDER'], filename))
                    screenshot = filename
                except Exception as e:
                    log_failure_kenzie(request.remote_addr, "Screenshot upload failed")
        ...
        if form.errors:
            ...
            for field, errors in form.errors.items():
                for error in errors:
                    log_failure_kenzie(request.remote_addr, f"{field} validation error: {error}")
...
def view_errors():
    failures = commentfailure.query.order_by(commentfailure.id.asc()).all()
    return render_template('view_errors.html', failures=failures)
    

@app.route('/dataPrivacy')
def data_privacy():
    return render_template('DataCollectionPolicy.html')
